...
function VIPContent() {
    ...
    // 🔑 VIP 갤러리 고유 ID - 각 VIP 페이지마다 다른 ID 사용
    ...
    const needsPayment = isPaymentRequired(); // 동적 결제 필요 여부 (Always Free 모드 대응)
    ...
    const handleKakaoShare = async () => {
        const shareUrl = window.location.href; // 현재 VIP 페이지 URL
        const title = `[VIP] ${settings.artistName} 작가님의 온라인 화첩`;
        const description = `프리미엄 구독 전용 공간입니다.`;
        if (navigator.share) {
            try {
