...
export default function VIPPageClient({ VIP_ID, isAlwaysFree = false }: VIPPageClientProps) {
    ...
    const shuffledArtworks = useMemo(() => {
        ...
        return [...currentYearMonthArtworks].sort(() => Math.random() - 0.5);
    }, [currentYearMonthArtworks]);
            ...
            <PaymentGate forcedMode={isAlwaysFree ? "always_free" : "commercial"}>
                ...
                <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
                    {artworks.length === 0 ? (
                        ...
                        /* [디자인 복구] Masonry 레이아웃 적용: columns-4 활용 */
                        <div
                            className={`gap-6 sm:gap-8 ${settings.gridColumns === 1 ? 'columns-1 max-w-2xl mx-auto' :
                                settings.gridColumns === 3 ? 'columns-1 sm:columns-2 lg:columns-3' :
